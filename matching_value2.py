...
def match_excel_data():
    ...
    source_file = os.path.join(base_path, "4506_baris_koordinat_kosong_fixed.xlsx")  # File dengan data lengkap
    target_file = os.path.join(base_path, "4506_fixed.xlsx")        # File yang akan diupdate
    output_file = os.path.join(base_path, "4506_updated.xlsx")                     # File hasil
    
    try:
        # Membaca kedua file Excel dengan dtype string untuk Latitude dan Longitude
        print("Membaca file sumber...")
        df_source = pd.read_excel(source_file, dtype={'Latitude': str, 'Longitude': str})
        
        print("Membaca file target...")
        df_target = pd.read_excel(target_file, dtype={'Latitude': str, 'Longitude': str})
        ...
        for idx, row in df_target.iterrows():
            ...
            if idsbr_target in df_source_indexed.index:
                ...
                for col in columns_to_update:
                    ...
                    if (pd.isna(df_result.at[idx, col]) or df_result.at[idx, col] == '' or 
                        df_result.at[idx, col] is None or str(df_result.at[idx, col]).strip() == 'nan'):
                        if (not pd.isna(source_row[col]) and source_row[col] != '' and 
                            source_row[col] is not None and str(source_row[col]).strip() != 'nan'):
                            
                            # Khusus untuk Latitude dan Longitude, pastikan tetap string
                            if col in ['Latitude', 'Longitude']:
                                df_result.at[idx, col] = str(source_row[col])
                            else:
        ...
        # Simpan hasil ke file baru dengan mempertahankan tipe data string untuk koordinat
        print(f"\nMenyimpan hasil ke {output_file}...")
        
        # Pastikan kolom Latitude dan Longitude tetap string sebelum disimpan
        if 'Latitude' in df_result.columns:
            df_result['Latitude'] = df_result['Latitude'].astype(str)
        if 'Longitude' in df_result.columns:
            df_result['Longitude'] = df_result['Longitude'].astype(str)
            
        # Ganti 'nan' dengan nilai kosong jika diperlukan
        df_result = df_result.replace('nan', '')
